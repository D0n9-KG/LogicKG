...
class FastTreeComm:
    ...
    def _compute_sim_matrix(self, level_nodes):
        ...
        sim_matrix = self.struct_weight * structural_sim_matrix + (1 - self.struct_weight) * semantic_sim_matrix
        return sim_matrix
    ...
    def detect_communities(self, level_nodes, max_iter=1, merge_threshold=0.5, max_total_communities=None):
        ...
        sorted_clusters = sorted(initial_clusters.items(), key=lambda item: len(item[1]), reverse=True)
    ...
    def _refine_cluster(self, cluster_nodes, max_iter, merge_threshold):
        ...
        for _ in range(max_iter):
            ...
            cluster_similarities.sort(key=lambda item: item["similarity"], reverse=True)
    ...
    def create_super_nodes(self, comm_to_nodes: Dict[str, List[str]], level: int = 4, batch_size: int = 5):
        super_nodes = {}
        communities = [(comm_id, members) for comm_id, members in comm_to_nodes.items() if len(members) >= 2]
        ...
        for i in range(0, len(communities), batch_size):
            ...
            if self.llm_client:
                try:
                    ...
                    logger.error(f"Batch LLM processing failed: {exc}")
                    llm_dict = {}
            else:
                llm_dict = {}
            ...
            for comm_id, members in batch:
                try:
                    ...
                    logger.error(f"Error creating super node for community {comm_id}: {exc}")
    ...
    def extract_keywords_from_community(self, community_nodes: List[str], top_k: int = 5) -> List[str]:
        ...
        combined_scores = {
            node: self.struct_weight * norm_structural[node] + (1 - self.struct_weight) * norm_semantic[node]
            for node in community_nodes
        }

        top_nodes = sorted(community_nodes, key=lambda node: combined_scores[node], reverse=True)[:top_k]
        return top_nodes
    ...
    def create_super_nodes_with_keywords(self, comm_to_nodes: Dict[str, List[str]], level: int = 4, batch_size: int = 5):
        ...
        for comm_id, members in comm_to_nodes.items():
            ...
            try:
                ...
                logger.error(f"Error creating keywords for community {comm_id}: {exc}")
