...
def test_build_community_rows_ignores_remote_membership_artifacts() -> None:
    entities = [
        ...
        {"entity_id": "e-4", "name": "Velocity", "attributes": '{"community_id": 9}'},
    ]
    relations = [
    ...
    assert all(str(row["community_id"]).startswith("community:derived:") for row in communities)
