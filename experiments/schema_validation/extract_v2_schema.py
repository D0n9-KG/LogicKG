...
OUT = os.path.join(TMP, "schema_validation_results.jsonl")
...
SCHEMA_V2 = """Schema (three-tier, v2):
...
L3 higher-order:
  - FUNCTION_RELATION (constitutive/scaling law or governing equation). subtype: constitutive_law | empirical_scaling | governing_equation | numerical_relation. Nestable (an argument can be another function).
  - COMPARISON_ARM (comparison group, e.g. frictional vs frictionless; regimes).
  - CAUSAL_ATTRIBUTION (mechanism attribution: "X is primarily determined by Y").
...
L3: {"layer":"L3","type":"FUNCTION_RELATION|COMPARISON_ARM|CAUSAL_ATTRIBUTION|RESEARCH_QUESTION","subtype":"(for FUNCTION_RELATION)","statement":"...","params":[...],"applies_in":"..."}
...
def main() -> None:
    ...
    print(f"samples: {len(SAMPLES)} | model: {MODEL} | schema: v2")
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        ...
        for fut in as_completed(futs):
            ...
            results.append(r)
            if r.get("stage") == "done":
                print(f"  [{r['label']}] n={r['n_atoms']} layers={r['layers']} l3={r['l3_types']} fr_sub={r['fr_subtypes']} ptype={r['paper_type_inferred']}")
            else:
    ...
    # coverage table
    print("\n=== COVERAGE TABLE ===")
    print(f"{'paper':<22}{'n':>4}{'L1':>4}{'L2':>4}{'L3':>4}{'PAPER':>6}  L3-types  FR-subtypes  paper_type")
    for r in results:
        if r.get("stage") != "done":
            print(f"{r['label']:<22}  -- {r.get('stage')}")
            continue
        L = r["layers"]
        print(f"{r['label']:<22}{r['n_atoms']:>4}{L.get('L1',0):>4}{L.get('L2',0):>4}{L.get('L3',0):>4}{L.get('PAPER',0):>6}  {r['l3_types']}  {r['fr_subtypes']}  {r['paper_type_inferred']}")
